...
async function getCesta(req, res) {
  ...
  try {
    const cestaResult = await conn.query(
      'SELECT id FROM cesta WHERE usuario_id = $1',
      [usuarioId]
    );
    ...
    const itensResult = await conn.query(
      `SELECT ci.id, ci.produto_id, ci.quantidade,
              p.nome, p.preco, p.cor, p.imagem_url
       FROM cesta_itens ci
...
async function adicionarItem(req, res) {
  ...
  try {
    const produtoResult = await conn.query(
      'SELECT estoque FROM produto WHERE id = $1 AND status = true',
      [produto_id]
    );
    ...
    await conn.query(
      `INSERT INTO cesta_itens (cesta_id, produto_id, quantidade)
       VALUES ($1, $2, $3)
       ON CONFLICT (cesta_id, produto_id)
       DO UPDATE SET quantidade = cesta_itens.quantidade + $3`,
...
async function atualizarItem(req, res) {
  ...
  try {
    ...
    const result = await conn.query(
      ...
      [quantidade, cestaId, produto_id]
    );

    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Item não encontrado na cesta' });
    }
