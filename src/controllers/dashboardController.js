  ...
  try {
    ...
    const graficoResult = await conn.query(`
      ...
      SELECT
        ...
        COALESCE(md.receita, 0)        AS receita,
        COALESCE(md.pedidos, 0)        AS pedidos
      FROM months m
      LEFT JOIN monthly_data md ON m.mes = md.mes
      ORDER BY m.mes ASC
    `);
    ...
    const estoqueBaixoResult = await conn.query(`
      SELECT nome, estoque, feminino
      FROM produto
      WHERE status = true AND estoque <= 50
    ...
    const topProdutosResult = await conn.query(`
      SELECT
        p.nome,
        COALESCE(SUM(ci.preco * ci.quantidade), 0) AS receita,
        COUNT(DISTINCT ci.compra_id)               AS pedidos
      FROM produto p
    ...
    for (const row of statusResult.rows) {
      ...
      if (!statusAgrupado[mapped.label]) {
        statusAgrupado[mapped.label] = { label: mapped.label, color: mapped.color, quantidade: 0 };
      }
      statusAgrupado[mapped.label].quantidade += parseInt(row.quantidade);
    }
    const status_pedidos = Object.values(statusAgrupado).map(s => ({
      label: s.label,
      value: totalCompras > 0 ? Math.round((s.quantidade / totalCompras) * 100) : 0,
      color: s.color,
    }));

    const pedidos_recentes = recentesResult.rows.map(row => ({
      id:     `#GNJ-${String(row.id).padStart(5, '0')}`,
      cliente: row.cliente,
      itens:  parseInt(row.itens),
    ...
    const estoque_baixo = estoqueBaixoResult.rows.map(row => ({
      ...
      categoria: row.feminino ? 'Fem.' : 'Masc.',
    }));
