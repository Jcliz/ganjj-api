...
// Retorna o id de tipo_roupa pelo nome (case-insensitive), ou null
...
// Substitui todos os tamanhos de um produto
...
// Fragment de SELECT reutilizado em todas as queries de leitura
...
async function getProdutos(req, res) {
    try {
        const { feminino, tipo_roupa, tamanho, popular, preco_max } = req.query;
        ...
        if (feminino !== undefined) {
            const fem = parseBoolean(feminino);
            if (fem !== undefined) { conditions.push(`p.feminino = $${idx++}`); values.push(fem); }
        }
        if (tipo_roupa) {
            conditions.push(`tr.nome ILIKE $${idx++}`);
            values.push(tipo_roupa);
        }
        if (tamanho) {
            conditions.push(`EXISTS (SELECT 1 FROM produto_tamanhos pt WHERE pt.produto_id = p.id AND pt.tamanho = $${idx++})`);
            values.push(tamanho);
        }
        if (popular !== undefined) {
            const pop = parseBoolean(popular);
            if (pop !== undefined) { conditions.push(`p.popular = $${idx++}`); values.push(pop); }
        }
        const { novo, social } = req.query;
        if (novo !== undefined) {
            const n = parseBoolean(novo);
            if (n !== undefined) { conditions.push(`p.novo = $${idx++}`); values.push(n); }
        }
        if (social !== undefined) {
            const s = parseBoolean(social);
            if (s !== undefined) { conditions.push(`p.social = $${idx++}`); values.push(s); }
        }
        if (preco_max !== undefined) {
            const pmax = parsePreco(preco_max);
            if (pmax !== undefined) { conditions.push(`p.preco <= $${idx++}`); values.push(pmax); }
        }
...
async function createProduto(req, res) {
    try {
        ...
        const novoParsed     = parseBoolean(novo);
        const socialParsed   = parseBoolean(social);
        ...
        try {
            ...
            const result = await conn.query(
                ...
                [nome, descricao || null, precoParsed, estoqueParsed, cor || null,
                 statusParsed ?? true, imagem_url || null, popularParsed ?? false, femininoParsed ?? false,
                 novoParsed ?? true, socialParsed ?? false, tipo_roupa_id]
            );

            await salvarTamanhos(conn, result.rows[0].id, tamanhos);

            // Busca o produto com joins para retornar completo
...
async function updateProduto(req, res) {
    try {
        ...
        if (preco !== undefined) {
            ...
            if (p === undefined) return res.status(400).json({ error: 'Preço inválido' });
            updates.push(`preco = $${index++}`); values.push(p);
        }
        if (estoque !== undefined) {
            const e = parseEstoque(estoque);
            if (e === undefined) return res.status(400).json({ error: 'Estoque inválido' });
            updates.push(`estoque = $${index++}`); values.push(e);
        }
        ...
        if (status !== undefined) {
            const s = parseBoolean(status);
            if (s === undefined) return res.status(400).json({ error: 'Status inválido' });
            updates.push(`status = $${index++}`); values.push(s);
        }
        if (popular !== undefined) {
            const p = parseBoolean(popular);
            if (p === undefined) return res.status(400).json({ error: 'Valor inválido para popular' });
            updates.push(`popular = $${index++}`); values.push(p);
        }
        if (feminino !== undefined) {
            const f = parseBoolean(feminino);
            if (f === undefined) return res.status(400).json({ error: 'Valor inválido para feminino' });
            updates.push(`feminino = $${index++}`); values.push(f);
        }
        if (novo !== undefined) {
            const n = parseBoolean(novo);
            if (n === undefined) return res.status(400).json({ error: 'Valor inválido para novo' });
            updates.push(`novo = $${index++}`); values.push(n);
        }
        if (social !== undefined) {
            const s = parseBoolean(social);
            if (s === undefined) return res.status(400).json({ error: 'Valor inválido para social' });
            updates.push(`social = $${index++}`); values.push(s);
        }

        const conn = await db.connect();
        try {
            // tipo_roupa precisa de lookup antes de montar o UPDATE
            ...
            if (tamanhos !== undefined) {
                await salvarTamanhos(conn, id, tamanhos);
            }
